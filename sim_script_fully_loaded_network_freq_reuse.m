...
% - we assume base stations transmit using frequency ALOHA, i.e., each BS
%   transmits over one (randomly and uniformly selected) channel out of
%   "freq_reuse" channels
...
freq_reuse      = 1;            % Frequency ALOHA reuse factor 
...
for n_iter = 1:N_iteration
    ...
    p_active =  1/freq_reuse;                                               % probability of a BS being an interferer
    ...
    if numOfBSs > 0      
        ...
        ASE_vector(n_iter) = sum(active_BS_boolean)*log2(1+SIR_linear)/area/freq_reuse;     % copy Area Spectral Efficiency (ASE) for this snapshot into the ASE vector         
